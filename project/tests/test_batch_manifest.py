...
def test_create_and_load_batch_manifest(tmp_path):
    tasks = [
        ...
        },
        {
            'input': {
                ...
                'title': 'Bai viet 2',
                'domain': 'vietstock.vn',
                'fetch_ts': '2026-09-04T14:00:00+07:00',
            }
        },
    ]
    ...
    assert manifest['articles'][0]['article_id'] == 'art_001'
    assert manifest['articles'][1]['article_id'] == 'art_002'
...
def test_archive_task_packet(tmp_path):
    ...
    assert not (task_dir / 'aid_1.task.json').is_file()
    assert (task_dir / 'aid_2.task.json').is_file()

    cnt = archive_completed_tasks(['aid_2', 'non_existing'], task_dir, archive_date='20260907')
    assert cnt == 1
    assert not (task_dir / 'aid_2.task.json').is_file()
