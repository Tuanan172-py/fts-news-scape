...
def test_build_scheduler_jobs():
    ...
    sched = build_scheduler(
        ...
        cfg,
        lambda: calls.append("reclaim"),
    )
    jobs = {j.id: j for j in sched.get_jobs()}
    assert set(jobs) == {"capture", "derive", "drift", "reclaim"}
...
def test_scheduler_khong_con_job_dinh_tuyen_l1_code_first():
    """Job `l1_route` của lane cũ đã gỡ: không cấu hình nào bật lại được nó."""
