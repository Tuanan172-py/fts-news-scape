...
if __name__ == "__main__":
    ...
    p.add_argument("--today", action="store_true", help="chỉ bài fetch hôm nay (giờ VN)")
    p.add_argument("--days", type=int, help="N ngày gần nhất theo fetched_at")
    p.add_argument("--domain", nargs="+", dest="domains", help="lọc theo nguồn (vd: cafef fireant)")
    p.add_argument("--with-symbols", action="store_true", dest="with_symbols",
    ...
    args = p.parse_args()
    export(today=args.today, days=args.days, domains=args.domains,
           with_symbols=args.with_symbols, limit=args.limit, out=args.out, verbose=True)
