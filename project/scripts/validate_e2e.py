...
_HTML = (
    ...
    + "<p>Nội dung bài viết đủ dài để trafilatura bóc tách nội dung chính. </p>"
    * 20
...
def _make_bronze(tmp: Path) -> str:
    ...
    meta = {
        ...
        "fetch_ts": "2026-08-13T10:00:05+07:00",
        ...
        "images": [
            {
                "outer_tag": "<img>",
                "resolved_url": "https://cdn.example.vn/a.jpg",
                "alt": "minh hoạ",
                "title": "",
    ...
    (d / f"{hash_}.meta.json").write_text(
...
def main() -> int:
    ...
    store.insert(
        Article(
            url="https://example.vn/tin/kiem-thu.html",
    ...
    checks.append(
        (
            "enqueued pending",
            res["enqueue_status"] == "pending",
            str(res["enqueue_status"]),
        )
    )
    ...
    checks.append(
        (
            "catalog claim works",
            claimed is not None and claimed["article_id"] == hash_,
    ...
    if claimed:
        ...
        checks.append(
            ("mark_done → done", cat.counts().get("done", 0) == 1, str(cat.counts()))
        )

    # agent-output sample validates
    sample = json.loads(
        (SCHEMAS / "samples" / "agent-output-sample.json").read_text(encoding="utf-8")
    )
